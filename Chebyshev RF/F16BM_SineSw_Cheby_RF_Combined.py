...
validation_level = 2  # Level to use for validation 
location_i = 2  # DOF i
location_j = 3  # DOF j
...
# Keep only the levels requested plus validation level
all_needed_levels = levels_to_compute + [validation_level]
data_dict = {lvl: data_dict[lvl] for lvl in all_needed_levels}
...
# ----------- PROCESS DATA -----------
# Store processed data for all levels
processed_data = {}

print("-"*60)
print("PROCESSING DATA FOR ALL LEVELS")
print("-"*60)
...
for level in levels_to_compute:
    ...
    # Store all processed data for this level
...
# Build Chebyshev basis for all levels
for level in levels_to_compute:

    # Scale relative states to [-1,1]
    x_rel = processed_data[level]['x_rel']
    v_rel = processed_data[level]['v_rel']
    
    x_scaled, x_min, x_max = scale_to_unit_interval(x_rel)
    v_scaled, v_min, v_max = scale_to_unit_interval(v_rel)
    
    # Store scaled data back into processed_data
    processed_data[level]['x_scaled'] = x_scaled
    processed_data[level]['v_scaled'] = v_scaled
    
    # Build 2D Chebyshev basis
    H_poly = build_2d_chebyshev_basis(x_scaled, v_scaled, poly_order)
    
    # Store basis matrix back into processed_data
    processed_data[level]['H_poly'] = H_poly

    # Linear least squares: f_rest = H_poly @ alpha
    alpha, *_ = np.linalg.lstsq(H_poly, f_rest, rcond=None)

    # Store weights back into processed_data
    processed_data[level]['alpha'] = alpha

print("-"*60)
print("CREATING COMBINED MODEL")
print("-"*60)
# Average the coefficients
alphas = [processed_data[level]['alpha'] for level in levels_to_compute]
alpha_combined = np.mean(alphas, axis=0)
print(f"Combined model: Averaged {len(alpha_combined)} coefficients from {len(levels_to_compute)} levels")

# ----------- PREDICT RESTORING FORCE USING CHEBYSHEV POLYNOMIALS -----------
F_pred = H_poly @ alpha_combined

# ----------- VALIDATION DATA PROCESSING -----------
data = data_dict[validation_level]
...
# Compute relative motion using band-limited FFT integration
x_rel_val, v_rel_val = compute_relative_motion(a_i_bp, a_j_bp, fs, freq_low, freq_high)

# Compute restoring force
f_rest_val   = compute_restoring_force(a_i_bp, m_eff)

# Store all processed data for this level
processed_data[validation_level] = {
    'x_rel': x_rel,
    'v_rel': v_rel,
    'f_rest': f_rest,
}
...
eps_v = max(vel_thresh_frac * np.max(np.abs(v_rel_val)), vel_thresh_min)
eps_x = max(disp_thresh_frac * np.max(np.abs(x_rel_val)), disp_thresh_min)

# Create masks for slicing
# Stiffness slice (R vs displacement): near-zero velocity
near_zero_vel_mask = np.abs(v_rel_val) <= eps_v
# Damping slice (R vs velocity): near-zero displacement
near_zero_disp_mask = np.abs(x_rel_val) <= eps_x

# ----------- VISUALIZE SLICE PLOTS -----------
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))

# Stiffness plot (Restoring Force vs Displacement)
ax1.plot(x_rel_val[near_zero_vel_mask], f_rest_val[near_zero_vel_mask], 
         'bo', markersize=3, alpha=0.6, markerfacecolor='none', 
         markeredgewidth=0.8, label='Measured')
ax1.plot(x_rel_val[near_zero_vel_mask], F_pred[near_zero_vel_mask], 
         'ro', markersize=2, alpha=0.8, label='Chebyshev Fit')
ax1.set_xlabel('Relative Displacement [m]')
ax1.set_ylabel('Restoring Force [m/s²]')
ax1.set_title(f'(a) Stiffness Characteristic - Validation Level {validation_level}')
...
ax2.plot(v_rel_val[near_zero_disp_mask], f_rest_val[near_zero_disp_mask], 
         'bo', markersize=3, alpha=0.6, markerfacecolor='none', 
         markeredgewidth=0.8, label='Measured')
ax2.plot(v_rel_val[near_zero_disp_mask], F_pred[near_zero_disp_mask], 
         'ro', markersize=2, alpha=0.8, label='Chebyshev Fit')
ax2.set_xlabel('Relative Velocity [m/s]')
ax2.set_ylabel('Restoring Force [m/s²]')
ax2.set_title(f'(b) Damping Characteristic - Validation Level {validation_level}')
